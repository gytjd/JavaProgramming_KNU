...
public class GameDealer {
	...
	LinkedList<Card> cards=new LinkedList<Card>(); // 각 Card 객체를 Collection(ArrayList, Vector, HashMap 등)에 저장하여 1벌의 카드를 생성
	...
	void shuffle() // shuffle 기능: 랜덤하게 카드의 순서를 섞음
	{
		Collections.shuffle(cards);
	}
	
	LinkedList<Card> deal() // deal 기능: 초기 생성된 53장의 카드를 랜덤하게 섞은 다음 컴퓨터(Compter)와 인 간(Human)에게 각 27장, 26장을 나누어 줌
	{
	...
	public void play_game()
	{
		...
		while(computerPlayer.playerCards.size()!=0 && humanPlayer.playerCards.size()!=0) // 두 명의 선수 중 한 명의 카드수가 0이 될 때까지 게임을 진행하고, 마지막에 Joker 를 가지고 있는 선수를 찾음
		{
			System.out.println("다음 단계 게임 진행을 위해 Enter 키를 누르세요!");
			menu=scanner.nextLine();
			
			if(menu==""+"") // Enter키를 입력 받으면 다음 단계로 진행하고, 각 선수들의 카드 현황을 화면에 출 력함
			{
				System.out.printf("\n\n\n\n<< %d 단계 >>\n\n",round++);
				...
				if((computerPlayer.playerCards.size()==1 && computerPlayer.playerCards.get(0).number=="Joker")||(humanPlayer.playerCards.size()==1 && humanPlayer.playerCards.get(0).number=="Joker"))
				{
