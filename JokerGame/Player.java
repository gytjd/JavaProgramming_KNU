...
public class Player {
	...
	LinkedList<Card> playerCards=new LinkedList<Card>(); // 처음 딜러로 부터 받은 카드 목록을 사용하는 Collection 객체에 저장함
	LinkedList<Card> openCards=new LinkedList<Card>(); //가지고 있는 카드 목록에서 같은 번호나 같은 이름(A,J,Q,K)가 있으면 최대 2 장까지 공개 카드 목록(별도의 Collection 객체)으로 이동 시킴
	...
	void display() // 두개의카드목록(숨긴카드목록,공개한카드목록)출력함수구현
	{
	...
	void set_openCards() // 동일한 번호의 카드가 3장 있는 경우, 2장만 제거 가능
	{
		int i,j;
	...
	//각 단계가 진행될 때마다, 상대방이 가지고 있는 카드를 1장 선택(랜덤 선택)해 서 자신의 카드에 추가하고
	//추가된 카드와 같은 번호나 같은 이름이 있으면 가 지고 있는 목록에서 삭제하고 공개 카드 목록으로 이동 시킴
	...
	{	
		...
		this.playerCards.add(tempA); // 자신이 선택한 상대방 카드를 자신의 카드 목록에 추가
		player.playerCards.remove(indexA); // 상대방이 선택한 카드 인덱스를 자신의 목록에서 삭제
